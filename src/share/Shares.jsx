...
export const OwnerShares = () => {
  ...
  const activeShares = sharesData?.shares?.filter((share) => !share.wishList.archivedAt)
  const achivedShares = sharesData?.shares?.filter((share) => share.wishList.archivedAt)
  return (
    <>
      <Paper sx={{ padding: 2, display: 'flex', flexDirection: 'column' }}>
        <Typography component='h1' variant='h4' sx={{ marginTop: 3, marginLeft: 3, marginRight: 3 }}>
          Shared with me
        </Typography>
        {loading && <CircularProgress sx={{ alignSelf: 'center' }} />}
        {!loading && !achivedShares?.length && !activeShares?.length && (
          <Typography sx={{ margin: 3 }}>Nothing has been shared with you yet.</Typography>
        )}
        {Boolean(activeShares?.length) && (
          <List>
            {activeShares.map(({ id: shareId, wishList }) => (
              <WishListListItem key={shareId} shareId={shareId} wishList={wishList} />
            ))}
          </List>
        )}
        {Boolean(achivedShares?.length) && (
          <Accordion>
            <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls='archived-shares-header' id='archived-shares-header'>
